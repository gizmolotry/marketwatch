...
def test_run_and_verify_receipt_cli(tmp_path,capsys):
    ...
    config=tmp_path/"runner.json"; config.write_text(json.dumps({"format":"repo-cartographer-pytest-runner/v1","allowed_test_paths":["tests"],"timeout_seconds":30,"max_output_bytes":4096}),encoding="utf-8")
    receipt=tmp_path/"receipt.json"
    assert main(["run-tests","--root",str(root),"--inventory",str(inventory),"--config",str(config),"--output",str(receipt),"--test-path","tests"])==EXIT_OK
    result=json.loads(capsys.readouterr().out); assert result["promotable"] is True
    assert main(["verify-receipt","--receipt",str(receipt),"--inventory",str(inventory),"--config",str(config)])==EXIT_OK
