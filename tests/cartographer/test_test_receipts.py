...
from repo_cartographer.test_receipts import run_pytest_receipt, verify_test_receipt
...
def _fixture(tmp_path, source: str, *, extra: dict[str, str] | None = None):
    ...
    config = PytestRunnerConfig("repo-cartographer-pytest-runner/v1", ("tests",), 30, 4096)
...
"""
    ...
    receipt = run_pytest_receipt(inventory, config, receipt_path, repository_root=root, test_uids=[uids["test_plain"], uids["test_param"]])
    ...
    assert verify_test_receipt(receipt_path, inventory, config)["receipt_sha256"] == receipt["receipt_sha256"]
...
def test_skip_xfail_xpass_setup_and_teardown_never_promote(tmp_path, body):
    ...
    receipt = run_pytest_receipt(inventory, config, tmp_path / "receipt.json", repository_root=root, test_uids=[uids["test_case"]])
...
"""
    root, inventory, uids, config = _fixture(tmp_path, source)
    receipt = run_pytest_receipt(inventory, config, tmp_path / "receipt.json", repository_root=root, test_uids=[uids["test_param"], uids["test_plain"]])
...
def test_source_mismatch_and_receipt_tampering_are_rejected(tmp_path):
    ...
    receipt_path = tmp_path / "receipt.json"
    run_pytest_receipt(inventory, config, receipt_path, repository_root=root, test_uids=[uids["test_case"]])
    ...
    with pytest.raises(ValueError, match="stale or mismatched"):
        run_pytest_receipt(inventory, config, tmp_path / "stale.json", repository_root=root, test_uids=[uids["test_case"]])
...
def test_dirty_but_byte_stable_inventory_can_run(tmp_path):
    ...
    receipt = run_pytest_receipt(inventory, config, tmp_path / "dirty-receipt.json", repository_root=root, test_uids=[test_uid])
    assert receipt["results"]["promotable"] is True
...
def test_dynamically_unmapped_nodeid_is_explicit_and_blocks_promotion(tmp_path):
    ...
    receipt = run_pytest_receipt(inventory, config, tmp_path / "unmapped.json", repository_root=root, test_uids=[uids["test_case"]])
...
def test_oversized_plugin_event_file_is_not_read_and_blocks_promotion(tmp_path):
    ...
    bounded = PytestRunnerConfig("repo-cartographer-pytest-runner/v1", ("tests",), 30, 1024)
    receipt = run_pytest_receipt(inventory, bounded, tmp_path / "overflow.json", repository_root=root, test_uids=[uids["test_case"]])
