...
|---|---|---|
| Raw capture | Content-addressed raw objects, retrieval receipts, quarantine, source coverage, canonical market records, and bounded exact-condition Polymarket public-trade population manifests | Continuous multi-source capture with source-specific gap recovery, immutable lake partitions, and complete coverage ledgers |
...
| Actor features | Public Polymarket `proxyWallet` aggregation over canonical fills: counts, notional, concentration, positions, and resolved performance; evidence-bound exact-condition, frozen-window population acquisition with immutable incomplete manifests and explicit `query_exhausted_coverage_limited`/`partial` status | Longitudinal pseudonymous actor sequences, novelty, burstiness, funding/settlement graph motifs, peer baselines, and explicit visibility masks |
| Observational corpus | Offline deterministic `TradeFill` freezer binding selected canonical rows, raw objects/receipts, coverage rows, and code; no labels or effectiveness claim | Frozen, independently governed multimodal/adjudicated corpus with declared retention, partitions, case mappings, and prospective holdouts |
...
For actor-population collection, Polymarket is presently the primary public source: the Data API exposes a venue `proxyWallet` on public trades. Kalshi has market/trade/book controls but no public actor identity, so it cannot provide a Kalshi wallet cohort. The implemented Polymarket collector fixes one condition ID, both participant-role coverage (`takerOnly=false`), a whole-second start/end interval, and a recursively partitioned query tree. It requires two raw/receipt-bound captures—an operator-approved official `/trades` contract snapshot and exact Gamma market metadata. Gamma `acceptingOrdersTimestamp` supplies a market-activity lower bound only; Data API retention remains unknown/approximate. Each fetched leaf binds every response-bearing retry attempt and the collector separately bounds logical requests and total HTTP attempts. The final successful raw rows must exactly equal production-parser output, including actor and lineage fields; malformed source wallets and non-exact source sides fail closed. Saturated side leaves remain `irreducibly_partial`; request/attempt/leaf limits create explicit `budget_exhausted` leaves; inconsistent parent/child reconciliation is preserved. Because the exact market-query retention floor is unknown, `complete` is always false: a fully exhausted, consistent query is only `query_exhausted_coverage_limited`, otherwise `partial`. Unfetched budget intervals are explicit but never forged into the coverage ledger, and persisted ledger rows remain incomplete. Polygon facts remain an exact-fill corroboration channel, never a proxy wallet-attribution channel.

The local 630,277-fill exploratory corpus is an uncommitted observational artifact, not a checked-in dataset or training result. All 238 relevant coverage records are incomplete. Its deterministic freeze can attest its local file/receipt/ledger state while retaining `ready_with_limitations`; it cannot serve as a complete population, a fraud-labelled corpus, or evidence that the system is effective.
...
| Prediction-market actor | Polymarket public `proxyWallet` where exposed, actor visibility, fills, direction, price, size, market/outcome, time; Kalshi actor unavailable | Fill/market/outcome counts; gross notional; directional concentration; per-outcome net size, gross buys/sells, cash flow; resolved performance; exact single-condition population manifests and coverage limitations | Actor age; prior activity rate; burstiness; market/category novelty; position size versus own history; timing to event; entry/exit/reversal behavior; cross-market concentration; funding recency; coordinated temporal motifs; peer-normalized behavior; visibility and history-length masks |
...
- Use the implemented single-condition Polymarket population collector for bounded public-wallet cohorts; retain its approved contract/Gamma evidence bound, recursive query-tree manifest, source reconciliation, and explicit irreducible/budget partiality instead of treating a saturated page as a population.
...
The current observational freezer is a prerequisite for this phase, not evidence that it has begun. A frozen local file hash and a large row count do not supply adjudicated labels, complete coverage, trained parameters, or a validated score.
